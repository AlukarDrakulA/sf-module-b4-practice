...
# запрос идентификатора пользователя у юзера
def request_data():
    user_id = input('Введите идентификатор пользователя на которого хотите найти похожих атлетов: ')
    return int(user_id)
...
def near_birthday(user, session):
...
def main():
    ...
    if not user:
        ...
        bd_athlete, bd = near_birthday(user, session)
        height_athlete, height = near_height(user, session)
        print(
